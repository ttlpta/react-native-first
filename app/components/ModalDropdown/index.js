...
export default class ModalDropdown extends Component {
  ...
  _renderRow(rowData, sectionID, rowID, highlightRow) {
    let key = `row_${rowID}`;
    let dropdown_icon = rowData === 'vi' ? require('../../assets/images/Vietnam-Flag-icon.png') : require('../../assets/images/Kingdom-flag-icon.png');
    let highlighted = rowID == this.state.selectedIndex;
    let row = !this.props.renderRow ?
      (<View style={{flexDirection: 'row'}}><Image style={styles.dropdown_image}
...
const styles = StyleSheet.create({
  ...
  dropdown: {
    ...
    ...Platform.select({
      ios: {
        height: (13 + StyleSheet.hairlineWidth) * 5,
      },
      android: {
        height: (15 + StyleSheet.hairlineWidth) * 5,
      },
    }),
  },
  loading: {
